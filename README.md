...
| Real (non-fabricated) vulnerability findings through the full pipeline | **Verified across two ecosystems**: a triggered CI run against `demo-target/`'s deliberately-old pinned packages reported 171 real findings (120 Python + 51 npm), 0 pre-suppressed, flowing through Syft → Grype → VEX filter → signed report. (An earlier attempt to add npm coverage via a bare `package.json` produced zero additional findings — Syft's javascript cataloger needs `package-lock.json`; caught by comparing artifact sizes across real runs rather than assuming it worked, see `demo-target/README.md`.) |
